...
for(variable in 1:2){
	for(advec_config in 1:3){
		cat(variable, advec_config, '\n')
		realizations_example <- read.table(paste(root, 'Data/univariate-nonstationary/realizations-example-6-velocity_mu_config_', velocity_mu_config, '_velocity_var_config_', velocity_var_config, "_advec_config_", advec_config, sep = ''), header = FALSE, sep = " ") %>% as.matrix()
		REALIZATIONS_MAT <- rbind(REALIZATIONS_MAT, realizations_example[4, (variable - 1) * n * TT + 1:(n * TT)])
	}
}

movie_simulated_data_for_beamer(realizations = REALIZATIONS_MAT, locations = sim_grid_locations, file_name = '0-univariate-nonstationary-cov6', row_labels = c(expression(Z[1]), expression(Z[2])))
