...
pdf(file = paste(root, 'Figures/4_application.pdf', sep = ''), width = 25, height = 4)

split.screen( rbind(c(0.05,0.95,0.1,0.95), c(0.95,0.99,0.1,0.95)))
...
for(hr in hr_ind){
	...
	quilt.plot(DAT_SUB[, 1], DAT_SUB[, 2], DAT_SUB[, 2 + hr], zlim = zlim_range, nx = 600, ny = 100, ylab = '', xlab = '', cex.lab = 4, add.legend = F, cex.axis = 2, axes = F)
...
legend.gradient2(cbind(x1,y1), title = "", limits = round(seq(zlim_range[1], zlim_range[2], length.out = 3), 1), CEX = 1.5)
