...
for(MODEL in 1:2){
	...
	for(m in 1:length(adj_mu)){
		...
		if(SIMULATION1){
			WIND_SIMULATED <- mvrnorm(n_sim, mu = WIND_MU + adj_mu[m], Sigma = matrix(WIND_VAR * adj_sig[m], ncol = 2, nrow = 2))
		}else if(SIMULATION2){
			if(m == 1){
				WIND_SIMULATED <- mvrnorm(n_sim, mu = WIND_MU, Sigma = WIND_VAR)
			}else{
				WIND_SIMULATED <- rmsn(n = n_sim, xi = WIND_MU, WIND_VAR, alpha = c(0, 0) + c(adj_alpha[m], 0))
			}
		}
...
if(PLOT){
	...
	par(mfrow = c(2, 3))
	image.plot(matrix(theo[65, 1:n], N, N))
	image.plot(matrix(theo[150, 1:n], N, N))
	image.plot(matrix(theo[321, 1:n], N, N))
	#image.plot(matrix(theo[65, n + 1:n], N, N))
	#image.plot(matrix(theo[65, 2 * n + 1:n], N, N))

	image.plot(matrix(basis_fn_mat[65, 1:n], N, N))
	image.plot(matrix(basis_fn_mat[150, 1:n], N, N))
	image.plot(matrix(basis_fn_mat[321, 1:n], N, N))
	#image.plot(matrix(basis_fn_mat[65, n + 1:n], N, N))
	#image.plot(matrix(basis_fn_mat[65, 2 * n + 1:n], N, N))
	dev.off()
...
basis_fn_vals <- list()
DIST <- distR_C(sim_grid_locations, X)
basis_fn_vals[[1]] <- exp(- DIST / 0.1)
DIST <- distR_C(sim_grid_locations, X - 0.075)
basis_fn_vals[[2]] <- exp(- DIST^2 / 0.1)
DIST <- distR_C(sim_grid_locations, X - 0.15)
basis_fn_vals[[3]] <- exp(- DIST / 0.5)
DIST <- distR_C(sim_grid_locations, X - 0.225)
basis_fn_vals[[4]] <- exp(- DIST^2 / 0.5)


theocov <- THEOCOV[, , 1]
theo <- theocov[1:n, 1:n]
...
NEGLOGLIK <- function(p){
	...
	basis_fn_mat <- matrix(0, n, n)

	for(aa in 1:3){
		PARAMETER_NONSTAT <- matrix(t(sigma) %*% parWarpsSum[, aa], byrow = T, nrow = 400, ncol = nrow(parWarpsSum))

		basis_fn_vals_new <- basis_fn_vals[[aa]] * PARAMETER_NONSTAT
		basis_fn_mat <- basis_fn_mat + basis_fn_vals_new %*% t(basis_fn_vals_new)
	}
	out <- sum((theo - basis_fn_mat)^2)
...
i = 1
set.seed(i)
init <- runif( 3 * length(jWarp), -0.001, 0.001)
fit1 <- optim(par = init, fn = NEGLOGLIK, control = list(trace = 5, maxit = 3000)) #
i = i + 1

for(dd in 1:100){
	fit1 <- optim(par = fit1$par, fn = NEGLOGLIK, control = list(trace = 5, maxit = 3000)) #
}

jWarp = 1:15
fit1 <- optim(par = c(fit1$par, rep(0, 5)), fn = NEGLOGLIK, control = list(trace = 5, maxit = 3000)) #
...
NEGLOGLIK <- function(p){
	...
	PARAMETER_NONSTAT <- matrix(t(sigma) %*% parWarpsSum, byrow = T, ncol = ncol(basis_fn_vals[[1]] ), nrow = nrow(basis_fn_vals[[1]] ))

	basis_fn_vals_new <- basis_fn_vals[[1]] * PARAMETER_NONSTAT
	basis_fn_mat <- basis_fn_vals_new %*% t(basis_fn_vals_new)
...
i = 1
set.seed(i)
init <- runif( length(jWarp), -0.001, 0.001)
fit1 <- optim(par = init, fn = NEGLOGLIK, control = list(trace = 5, maxit = 3000)) #
fit1 <- optim(par = fit1$par, fn = NEGLOGLIK, control = list(trace = 5, maxit = 3000)) #


NEGLOGLIK2 <- function(p){
...
fit2 <- optim(par = rep(0, length(jWarp)), fn = NEGLOGLIK2, control = list(trace = 5, maxit = 3000)) #
fit2 <- optim(par = fit2$par, fn = NEGLOGLIK2, control = list(trace = 5, maxit = 3000)) #





NEGLOGLIK2_b <- function(p){
...
fit2_b <- optim(par = c(fit1$par, fit2$par), fn = NEGLOGLIK2_b, control = list(trace = 5, maxit = 3000)) #
fit2_b <- optim(par = fit2_b$par, fn = NEGLOGLIK2_b, control = list(trace = 5, maxit = 3000)) #



NEGLOGLIK3 <- function(p){

	beta1 <- fit2_b$par[1:length(jWarp)]
	beta2 <- fit2_b$par[length(jWarp) + 1:length(jWarp)]
	beta3 <- p
...
fit3 <- optim(par = rep(0, length(jWarp)), fn = NEGLOGLIK3, control = list(trace = 5, maxit = 3000)) #
fit3 <- optim(par = fit3$par, fn = NEGLOGLIK3, control = list(trace = 5, maxit = 3000)) #


NEGLOGLIK3_b <- function(p){
...
fit3_b <- optim(par = c(fit2_b$par, fit3$par), fn = NEGLOGLIK3_b, control = list(trace = 5, maxit = 3000)) #
fit3_b <- optim(par = fit3_b$par, fn = NEGLOGLIK3_b, control = list(trace = 5, maxit = 3000)) #

p <- fit3_a$par

NEGLOGLIK4 <- function(p){
...
fit4_a <- optim(par = c(fit3_a$par, fit4$par), fn = NEGLOGLIK4, control = list(trace = 5, maxit = 3000)) #
fit4_a <- optim(par = fit4_a$par, fn = NEGLOGLIK4, control = list(trace = 5, maxit = 3000)) #
...
NEGLOGLIK4 <- function(p){
	...
	beta1 <- fit3_a$par[1:length(jWarp)]
	beta2 <- fit3_a$par[length(jWarp) + 1:length(jWarp)]
	beta3 <- fit3_a$par[2 * length(jWarp) + 1:length(jWarp)]
	beta4 <- p
...
fit4 <- optim(par = rep(0, length(jWarp)), fn = NEGLOGLIK4, control = list(trace = 5, maxit = 3000)) #
fit4 <- optim(par = fit4$par, fn = NEGLOGLIK4, control = list(trace = 5, maxit = 3000)) #
