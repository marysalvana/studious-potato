...
MATERN_UNI_DEFORMATION <- function(PARAMETER, PARAMETER_DEFORMATION, LOCATION, TIME, N_SIM = NULL, FITTING = F, PARALLEL = F, DIST) {
	...
	if(!FITTING & !PARALLEL){
		...
		FINAL_DATA <- list("covariance" = SIGMA[[1]], "parameters" = SIGMA[[2]])
                ...
                SIGMA <- DEFORMATION_PARALLEL(Loc = LOCATION_NEW, param = PARAMETER[1:3], wind = WIND_SIMULATED, param_nonstat = PARAMETER_DEFORMATION)

                FINAL_DATA <- SIGMA
        }else if(FITTING & !PARALLEL){
		SIGMA <- DEFORMATION_FOR_FITTING(Loc = LOCATION_NEW, param = PARAMETER[1:3], wind = WIND_SIMULATED, param_nonstat = PARAMETER_DEFORMATION)

		FINAL_DATA <- list("covariance" = SIGMA[[1]])
	}else if(FITTING & PARALLEL){
                SIGMA <- DEFORMATION_FOR_FITTING_PARALLEL(Loc = LOCATION_NEW, param = PARAMETER[1:3], wind = WIND_SIMULATED, param_nonstat = PARAMETER_DEFORMATION)
